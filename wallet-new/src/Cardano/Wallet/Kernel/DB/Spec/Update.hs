...
newPending :: forall c. IsCheckpoint c
           => InDb Txp.TxAux
           -> Update' (Checkpoints c) NewPendingFailed ()
...
newForeign :: forall c. IsCheckpoint c
           => InDb Txp.TxAux
           -> Update' (Checkpoints c) NewForeignFailed ()
...
applyBlockPartial :: PrefilteredBlock
                  -> Update' (Checkpoints PartialCheckpoint)
                             ApplyBlockFailed
                             (Set Txp.TxId)
...
rollback :: Update' (Checkpoints Checkpoint) e Pending
rollback = state $ \case
...
observableRollbackUseInTestsOnly :: Update' (Checkpoints Checkpoint) e Pending
